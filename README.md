...
2. **极简命令系统**：仅需使用 12 个核心命令（增加智能代码分析）
3. **中文原生交互**：与 AI 进行自然流畅的中文对话
4. **Context使用效率优化**：智能按需读取文档，Context使用效率提升60-75%
5. **数据库自动备份**：开发前智能检测并主动备份数据库，防止数据丢失
6. **Git 分支隔离**：自动创建功能分支，保护主分支不受影响
7. **会话连续性**：跨会话无缝保持项目上下文，支持断点恢复
8. **进度实时跟踪**：任务状态实时更新，自动持久化保存
...
├── kiro-analyze.md       # 🔍 代码分析 - 包含AI智能代码分析和专家选择逻辑
...
3. **验证安装**
```bash
ls .claude/commands/      # 应该显示12个自包含的命令文件
...
│       ├── kiro-analyze.md    # 🔍 代码分析（包含AI智能分析和专家选择）
